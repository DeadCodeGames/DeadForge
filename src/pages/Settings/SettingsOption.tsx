...
const SettingsOption = ({ title, description, controls }: { title: string, description?: string | React.JSX.Element, controls?: React.JSX.Element }) => {
    ...
    return (
        <div className="flex flex-row settingsShrink:flex-col gap-y-4 justify-between items-center settingsShrink:text-center gap-x-4 mb-4 p-4 bg-opacity-5 bg-notQuiteBlack dark:bg-opacity-5 dark:bg-notQuiteWhite rounded-lg">
            <div className={`flex flex-col ${(isBelow1000 && controls?.type.name === FlipSwitch.name) ? FlipSwitchSpacing : ""}`}>
                {title && (<div className="font-bold flex flex-row settingsShrink:justify-center items-center gap-x-4">{title} { (isBelow1000 && controls?.type.name === FlipSwitch.name) && controls }</div>)}
                {description && (<div className="text-opacity-70 text-notQuiteBlack dark:text-opacity-70 dark:text-notQuiteWhite italic whitespace-break-spaces">{description}</div>)}
            </div>
